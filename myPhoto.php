...
FROM traveluser
         RIGHT JOIN travelimage ON traveluser.UID = travelimage.UID
WHERE traveluser.UserName = :UserName ";
                    ...
                    while($row = $stmt->fetch())
                    {
                        ...
                        <li>
                            ...
                            <span class="image_right_area">
                                ...
                                <br/>
                                <button class="modify" value="修改" onclick="window.location.href='upload.php?imageID=<?php echo $row['ImageID'] ?>'">
                                    修改
                                </button>
                                ...
                                <script>
                                    document.getElementById('pic_delete<?php echo $row['ImageID'] ?>').onclick = function()
                                    {
                                        ...
                                        {
                                            window.location.href = 'php/deletePicture.php?imageID=<?php echo $row['ImageID'] ?>';
                                        }
                                    }
                                </script>
                            </span>
                ...
                {
                    ?>
                    <strong class="image_title" style="height: 190px">您还没有上传任何图片。</strong>
                    <?php
                }
                ?>
            </ul>
            ...
            <div class="page_number">
                <a href="myPhoto.php?<?php echo changePageTo($_SERVER["QUERY_STRING"], max(1, $currentPage - 1)); ?>"
                   title="向前一页"><<</a>
                <?php
                for($i = 1; $i <= $totalPage; $i++)
                {
                    ...
                    <a <?php if($i == $currentPage) echo "class=\"active\"" ?>
                            href="myPhoto.php?<?php echo changePageTo($_SERVER["QUERY_STRING"], $i); ?>"
                            title="<?php echo $i ?>"><?php echo $i ?></a>
                    <?php
                }
                ?>
                <a href="myPhoto.php?<?php echo changePageTo($_SERVER["QUERY_STRING"], min($totalPage, $currentPage + 1)); ?>"
                   title="向后一页">>></a>
            </div>
            <?php
            $conn = null;
