...
function updateDatabase($PATH)
{
    try
    {
        ...
        $row = $stmt->fetch();
        $UID = $row['UID'];
        $sql = "UPDATE travelimage SET Title=:Title, Description=:Description, Latitude=:Latitude, Longitude=:Longitude,
CityCode=:CityCode, CountryCodeISO=:CountryCodeISO, UID=:UID, PATH=:PATH, Content=:Content
WHERE ImageID=:ImageID";//插入
        $stmt = $conn->prepare($sql);
...
{
    if($_FILES['picture']['name'] != '')//如果更改了图片
    {
        // 允许上传的图片后缀
